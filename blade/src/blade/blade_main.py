...
def is_svn_client(blade_root_dir):
    ...
    if os.path.exists(os.path.join(blade_root_dir, '.svn')):
         return True
    output = os.popen("svn info").read();
...
def _check_code_style(opened_files):
    ...
    try:
        ...
        if p.returncode:
            if p.returncode == 127:
                msg = "Can't execute '%s' to check style, you can config the 'cpplint' option to be a valid cpplint path in the 'cc_config' section of blade.conf or BLADE_ROOT, or make sure '%s' command is correct." % (cpplint, cpplint)
            else:
...
def _build(options):
    ...
    scons_exe = os.environ.get('SCONS_EXE', 'scons')
    p = subprocess.Popen("%s %s" % (scons_exe, scons_options), shell=True)
    try:
        p.wait()
        if p.returncode:
...
def clean(options):
    ...
    scons_exe = os.environ.get('SCONS_EXE', 'scons')
    p = subprocess.Popen(
    "%s --duplicate=soft-copy -c -s --cache-show" % scons_exe, shell=True)
    p.wait()
...
def cleanup(options):
    ...
    blade_root_dir = blade.blade.get_current_source_path()
    for name in ('build32_debug', 'build32_release','build64_debug', 'build64_release', 'blade-bin', 'ccache-dir', 'SConstruct', '.sconsign.dblite', '.SConstruct.lock', '.blade.test.stamp'):
        path = os.path.join(blade_root_dir, name)
        if os.path.isdir(path):
            console.info("remove:%s" % path)
            shutil.rmtree(path)
